...
pub fn spinner_frame(frame: u64) -> &'static str { SPINNER[(frame as usize) % SPINNER.len()] }
...
pub fn status_glyph(kind: ChapterKind, status: ChapterStatus, t: &Theme) -> (char, Color) {
    if matches!(kind, ChapterKind::ImageOnly) { return ('▣', t.status_image); }   // U+25A3
    if matches!(kind, ChapterKind::Empty)     { return ('–', t.ink_faint); }      // U+2013
    match status {
...
pub fn hairline_set() -> symbols::border::Set<'static> { symbols::border::ROUNDED }

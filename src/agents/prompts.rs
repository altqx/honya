...
7a. ตรวจทานก่อนส่งแบบบรรทัดต่อบรรทัด: เทียบ SOURCE_JP กับ translated_text อีกครั้งก่อนตอบเสมอ โดยเฉพาะชังก์ที่มีบทพูดติดกันหลายบรรทัด ตัวแบ่งฉาก หัวเรื่อง เครดิต หรือข้อความในวงเล็บ ต้องไม่ตกประโยคเปิด/ปิดหลัง `---` ไม่ตกชื่อภาค/ชื่อเรื่องย่อย ไม่ทำให้ประโยคท้ายขาดเครื่องหมายจบประโยค และห้ามทิ้งเครื่องหมายญี่ปุ่นเช่น `。` `？` `！` หรือวงเล็บเต็มรูป `（ ）` ไว้ในฉบับไทย เว้นแต่เป็นข้อความที่ตั้งใจให้ผู้อ่านเห็นเป็นญี่ปุ่นในเนื้อเรื่องจริง ๆ
...
2b. Japanese Residue and Sentence Endings: reject stray Japanese punctuation or full-width brackets (`。`, `、`, `？`, `！`, `（ ）`, `「 」`, `『 』`) in Thai prose unless the source is explicitly showing Japanese text inside the story. Also reject Thai narrative/dialogue sentences that drop the source sentence ending so the line reads unfinished.
...
pub fn build_reviewer_user(
    ...
    if !advisory_findings.is_empty() {
        ...
        s.push_str("These are hints, not verdicts: a number may be legitimately spelled out in Thai, Thai length varies, and a name may contain the same syllables as a flagged particle. For `วะ/ว่ะ`, reject only when it is an ordinary casual particle.\n");

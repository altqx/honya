...
4a. ความสม่ำเสมอของชื่อตัวละคร: ใช้ชื่อไทยหลัง → ในบล็อก CHARACTERS ให้ตรงตัวทุกครั้ง ห้ามสะกดต่างหรือบัญญัติชื่อไทยใหม่ให้ตัวละครที่มีชื่ออยู่แล้ว (ถ้ายังไม่มีในรายการ ให้เลือกการสะกดที่เป็นธรรมชาติแล้วใช้ให้คงเส้นคงวา) ตัวละครหนึ่งคนอาจถูกเรียกได้หลายชื่อ หากมีรายการ "เรียกอีกชื่อ" ให้ใช้คำแปลที่กำกับของแต่ละชื่อ (เช่น ユウ→ยู, お兄ちゃん→พี่) อย่ายุบรวมเป็นชื่อเดียว
...
7b. ตรวจชื่อและเสียงตัวละครกับ REFERENCE ทุกครั้งที่มีชื่อ/สรรพนาม: ก่อนส่งให้ไล่ชื่อคน คำเรียกอีกชื่อ สรรพนามบุรุษที่ 1/2 และคำลงท้ายใน translated_text เทียบกับ CHARACTERS.md หากชื่อมีหลายรูป เช่น ชื่อเต็ม นามสกุล ชื่อเล่น หรือคำเรียกเฉพาะผู้พูด ให้ใช้รูปไทยที่กำกับของผิวคำนั้น ไม่สลับกับคำเรียกของคนอื่นและไม่สะกดเองใหม่
7c. แก้จาก REVIEWER_FEEDBACK อย่างตรงจุด: เมื่อมี feedback จากรอบก่อน ให้แก้ทุกข้อที่ถูกระบุจริง ๆ แล้วอ่านทวนทั้งชังก์อีกครั้ง อย่าแก้เฉพาะคำแรกแล้วปล่อยคำสะกดผิด รูบิที่ยังไม่เกลา เครื่องหมายญี่ปุ่น หรือชื่อ/สรรพนามที่ feedback ชี้ไว้อีกตำแหน่งหนึ่งให้ค้างอยู่
7d. ความสัมพันธ์แบบซึ่งกันและกัน: คำอย่าง 互いに, お互い, 向け合う, 向き合う เมื่อใช้กับ 絆/関係/思い/気持 ต้องคงความหมายว่าทั้งสองฝ่ายมีต่อกันหรือผูกพันกัน ไม่แปลเป็นการ "มอบให้" แบบมอบสิ่งของ/ความรู้สึกฝ่ายเดียว ตัวอย่าง `あの二人が互いに向け合う以上の絆` ควรเป็น "สายสัมพันธ์ที่มากกว่าที่สองคนนั้นมีต่อกัน/ผูกพันกัน" ไม่ใช่ "สายสัมพันธ์ที่สองคนนั้นมอบให้กัน"
...
3b. Character Name Consistency: use each character's canonical Thai (after → in CHARACTERS) verbatim; reject a deviating spelling or a new Thai name for someone who already has one. For names listed under "เรียกอีกชื่อ" (e.g. ユウ→ยู, お兄ちゃん→พี่), the Thai must use that form's own rendering — reject a wrong or swapped alt-name rendering. Quote the wrong form and the correct one.
...
5b. Reciprocal Relation Fidelity: wording such as `互いに`, `お互い`, `向け合う`, or `向き合う` with bonds/feelings/relationships (`絆`, `関係`, `思い`, `気持`) is mutual/reciprocal. Reject Thai that makes it sound like a one-way gift or transfer, e.g. rendering `あの二人が互いに向け合う以上の絆` as `สายสัมพันธ์ที่สองคนนั้นมอบให้กัน`; prefer a shared/reciprocal phrasing like `มีต่อกัน`, `ผูกพันกัน`, or `มีให้กัน` when faithful.
...
6b. Casual Thai Particles: as the default style, rough sentence-final particles/interjections should use `ฟะ` or `เฟ้ย`, not `วะ` or `ว่ะ`. Reject ordinary casual uses of `วะ/ว่ะ`. `เว้ย` is allowed, especially fixed emphatic exclamations like `โธ่เว้ย`, but only sparingly for roughness required by the source, an established character voice, or a plot point; reject repeated or casual filler use of `เว้ย`.
...
pub fn build_reviewer_user(
    ...
    if !advisory_findings.is_empty() {
        ...
        s.push_str("These are hints, not verdicts: a number may be legitimately spelled out in Thai, Thai length varies, and a name may contain the same syllables as a flagged particle. For `วะ/ว่ะ`, reject only when it is an ordinary casual particle. For `俺` plus `ฉัน`, verify CHARACTERS.md/context before rejecting, and treat an explicit CHARACTERS.md `not ฉัน` rule as authoritative over continuity; for reciprocal-bond hints, reject only when Thai has actually made the relation one-way or gift-like.\n");

...
6c. คำลงท้ายห้วน/หยาบ: โดยค่าเริ่มต้นห้ามใช้ "วะ", "ว่ะ" หรือ "เว้ย" เป็นคำลงท้ายหรือคำอุทานหยาบ ให้ใช้ "ฟะ" หรือ "เฟ้ย" แทนเมื่ออยากได้น้ำเสียงห้วน สนิท โวยวาย หรือกวนแบบไลท์โนเวล อนุญาต "วะ/ว่ะ/เว้ย" เฉพาะกรณีที่หายากและสำคัญจริง ๆ เช่น ต้นฉบับต้องการความหยาบตรง ๆ เป็นจุดเด่นของตัวละคร หรือเป็นประเด็นของฉาก ห้ามใช้พร่ำเพรื่อ
...
6b. Casual Thai Particles: as the default style, rough sentence-final particles/interjections should use `ฟะ` or `เฟ้ย`, not `วะ`, `ว่ะ`, or `เว้ย`. Reject ordinary casual uses of `วะ/ว่ะ/เว้ย`. Allow them only when that exact roughness is rare and important to the source, an established character voice, or a plot point.
...
pub fn build_reviewer_user(
    ...
    if !advisory_findings.is_empty() {
        ...
        s.push_str("These are hints, not verdicts: a number may be legitimately spelled out in Thai, Thai length varies, and a name may contain the same syllables as a flagged particle. For `วะ/ว่ะ/เว้ย`, reject only when it is an ordinary casual particle; allow rare important uses required by SOURCE_JP or established character voice.\n");

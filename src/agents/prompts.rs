...
4. รูบิ/ฟุริงานะและวงเล็บญี่ปุ่น: ข้อความที่ผ่าน Pre-process จะเขียนรูบิเป็นรูปแบบ "คำฐาน (เสียงอ่าน)" เช่น 漢字 (かんじ) ให้แปลความหมาย/ชื่อเป็นภาษาไทยและตัดวงเล็บญี่ปุ่นทิ้งเป็นค่าเริ่มต้น โดยเฉพาะชื่อคน สถานที่ ชมรม แผนก ตำแหน่ง และคำสามัญ ห้ามเขียนรูปแบบไทยตามด้วยญี่ปุ่น เช่น "สุดาตะ (さかた)", "เพื่อนสมัยเด็ก (おさななじみ)", "ชมรม (同好会)" หรือ "แผนก (部)" ยกเว้นเฉพาะเมื่อรูปอักษร/เสียงอ่านญี่ปุ่นเป็นข้อมูลพล็อตที่ผู้อ่านต้องเห็นจริง ๆ เช่น อาเทจิ มุกอ่านสองชั้น จารึก ปริศนา หรือข้อความบนวัตถุ ให้ถ่ายทอดนัยเป็นภาษาไทยธรรมชาติและอ้างรูปญี่ปุ่นอย่างประหยัด ไม่ทิ้งเป็นวงเล็บญี่ปุ่นลอย ๆ
...
2a. Ruby/Furigana Resolution: source ruby is pre-rendered as `Base (Reading)`, e.g. 漢字 (かんじ). The Thai must convey the base/name/term in Thai and drop Japanese parentheticals by default, including ordinary names, places, clubs, departments, titles, and common nouns. Reject Thai followed by raw Japanese such as `สุดาตะ (さかた)`, `เพื่อนสมัยเด็ก (おさななじみ)`, `ชมรม (同好会)`, or `แผนก (部)`. Allow source Japanese only when the glyph/reading itself is plot-critical (ateji, a double-reading pun, inscription, riddle, written text the reader must see), and even then it must be integrated sparingly as Thai prose rather than leftover ruby/kanji/kana in parentheses.

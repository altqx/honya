...
pub const ORDER: &[Def] = &[
    ...
    def(SField::RetryAttempts, "HTTP retry attempts", Group::Pipeline, Kind::Number { min: 1, max: 10 },
        "Sends per call after a timeout, a dropped connection or a 5xx. 1 never retries; a rate limit gets a deeper budget."),
    def(SField::RetryCooldown, "Retry cooldown (s)", Group::Pipeline, Kind::Number { min: 1, max: 120 },
        "The longest one backoff waits. Caps the provider's own Retry-After hint too, so a run cannot stall for minutes."),

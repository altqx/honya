...
Each screen module (`shelf.rs`, `project.rs`, `translate.rs`, `reader.rs`, `lexicon.rs`) owns its own sub-state, `handle_key` (returns an `Action`), `render`, and `hints`. **`handle_key` and `render` are *served* the resolved action table** (`&[Act]`) by `App` rather than rebuilding it from their own `actions()`: `App::screen_actions` is the one place user bindings are folded in, and a screen that built its own would dispatch the original key alongside the rebound one and print the original on its toolbar.
